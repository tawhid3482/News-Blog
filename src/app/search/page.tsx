    ...
    return (
      <div className="flex flex-col items-center justify-center h-[80vh]">
        <p className="text-4xl font-bold text-[#0896EF]">
          No news found for this search result.
        </p>
      </div>
    );
  }

  const mainNews = searchNews[0];
  if (!mainNews) return <p>No news found.</p>;

  const relevantNews = searchNews.slice(1, 4);
  const excludedIds = [
    mainNews.id,
    ...relevantNews.map((newsItem: any) => newsItem.id),
  ];
  const otherNews = searchNews.filter(
        ...
        <section className="max-w-7xl mx-auto px-4 py-10">
          ...
          {otherNews.length > 0 && (
            <div className="mt-12 ">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 h-96">
